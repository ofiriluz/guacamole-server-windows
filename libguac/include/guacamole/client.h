...
struct guac_client {
     ...
     */
#ifdef HAVE_BOOST
	rwlock::RWLock<rwlock::SLEEP> __users_lock;
    ...
    */
     ...
     */
#ifdef HAVE_BOOST
	boost::shared_ptr<boost::dll::shared_library> __plugin_handle;
	/**
	* The client FPS of processing each frame, if set to 0 there will be no rate between each frame
	* Added by CA
	*/
	int client_fps;
#elif defined HAVE_LIBPTHREAD
