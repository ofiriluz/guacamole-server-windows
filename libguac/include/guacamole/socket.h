...
struct guac_socket {
    ...
    guac_socket_reset_handler* reset_handler;
     ...
     */
#ifdef HAVE_BOOST
	boost::shared_ptr<boost::thread> __keep_alive_thread;
#elif defined HAVE_LIBPTHREAD
 ...
 * @return Zero on success, or non-zero if an error occurs while writing.
 */
size_t guac_socket_write(guac_socket* socket, const void* buf, size_t count);
...
/**
 * Resets the socket to its initialized state
 * @param socket
 */
...
guac_socket* guac_socket_shared_memory_socket_create(const std::string & shname, bool multi_read, bool streamlined, int queue_size, int packet_size);
