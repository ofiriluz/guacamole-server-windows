...
size_t __guac_socket_write_length_string(guac_socket* socket, const char* str) {
...
size_t __guac_socket_write_length_int(guac_socket* socket, int64_t i) {

#ifdef HAVE_BOOST
	char buffer[128];
	snprintf(buffer, sizeof(buffer), "%lli", i);
...
size_t __guac_socket_write_length_double(guac_socket* socket, double d) {
...
#elif defined HAVE_LIBPTHREAD
    char buffer[128];
    snprintf(buffer, sizeof(buffer), "%.16g", i);
    return __guac_socket_write_length_string(socket, buffer);
#endif
}
