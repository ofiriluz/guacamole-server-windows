...
#ifdef HAVE_BOOST
    std::stringstream ss;
    ss << "audio/L" << audio->bps << ";rate=" << audio->rate << ",channels=" << audio->channels;
    guac_protocol_send_audio(socket, audio->stream, ss.str().c_str());
...
static void raw_encoder_begin_handler(guac_audio_stream* audio) {
    ...
    audio->data = state = static_cast<raw_encoder_state*>(malloc(sizeof(raw_encoder_state)));
    state->written = 0;
    state->length = GUAC_RAW_ENCODER_BUFFER_SIZE
                    * audio->rate * audio->channels * audio->bps
                    / 8 / 1000;

    state->buffer = (unsigned char * )malloc(state->length);
...
#ifdef HAVE_BOOST
guac_audio_encoder _raw8_encoder = {"audio/L8",
	raw_encoder_begin_handler,
	raw_encoder_write_handler,
	raw_encoder_flush_handler,
	raw_encoder_end_handler,
	raw_encoder_join_handler
};

/* 16-bit raw encoder handlers */
guac_audio_encoder _raw16_encoder = {"audio/L16",
	raw_encoder_begin_handler,
	raw_encoder_write_handler,
	raw_encoder_flush_handler,
	raw_encoder_end_handler,
