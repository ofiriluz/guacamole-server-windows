...
static boost::thread_specific_ptr<guac_status> __guac_error_tls;
static boost::thread_specific_ptr<char*> __guac_error_message_tls;

guac_status* __guac_error() 
{
	/* Init error, if not already initialized */
	if (!__guac_error_tls.get())
	{
		__guac_error_tls.reset(new guac_status);
	}
	return __guac_error_tls.get();
}

const char** __guac_error_message()
{
	/* Init error, if not already initialized */
	if (!__guac_error_message_tls.get())
	{
		__guac_error_message_tls.reset(new char *);
	}
	return (const char **)(__guac_error_message_tls.get());
}
#elif defined HAVE_LIBPTHREAD
