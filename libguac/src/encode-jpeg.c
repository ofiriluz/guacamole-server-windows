...
typedef struct guac_jpeg_destination_mgr {
    ...
    /**
     * The 
     over which all JPEG blobs will be written.
     */
    guac_socket* socket;
...
static void guac_jpeg_init_destination(j_compress_ptr cinfo) {
    ...
    dest->parent.next_output_byte = (JOCTET*)(dest->buffer);
...
int guac_jpeg_write(guac_socket* socket, guac_stream* stream,
    ...
    unsigned char *scanline_data = static_cast<unsigned char*>(malloc(write_stride));
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        row_pointer[0] = (JSAMPROW)scanline_data;
#endif
