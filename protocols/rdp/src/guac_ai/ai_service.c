...
static int guac_rdp_ai_new_connection(
    ...
    guac_rdp_ai_channel_callback* ai_channel_callback =
        static_cast<guac_rdp_ai_channel_callback*>(
...
static int guac_rdp_ai_initialize(IWTSPlugin* plugin,
    ...
    guac_rdp_ai_listener_callback* ai_listener_callback =
        static_cast<guac_rdp_ai_listener_callback*>(
...
int DVCPluginEntry(IDRDYNVC_ENTRY_POINTS* pEntryPoints) {
...
#else
    RDP_PLUGIN_DATA* data = (RDP_PLUGIN_DATA*)pEntryPoints->GetPluginData(pEntryPoints);
    guac_client* client = (guac_client*) guac_rdp_string_to_ptr((const char*)data->data[1]);
#endif
    ...
    if (ai_plugin == NULL) {
        ...
        ai_plugin = static_cast<guac_rdp_ai_plugin*>(
			calloc(1, sizeof(guac_rdp_ai_plugin)));
