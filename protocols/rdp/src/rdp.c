...
BOOL rdp_freerdp_pre_connect(freerdp* instance) {
    ...
    if (settings->printing_enabled
        ...
        ///* Load RDPDR plugin */
        //if (freerdp_channels_load_plugin(channels, instance->settings,
        //            "C:/CAWorkspace/PIM/PSM/PSM-Gateway/Main/Server/Guacamole/GuacBuild/protocols/rdp/RelWithDebInfo/guacdr", client))
        //    guac_client_log(client, GUAC_LOG_WARNING,
        ...
        ///* Load RDPSND plugin */
        //if (freerdp_channels_load_plugin(channels, instance->settings,
        //            "C:/CAWorkspace/PIM/PSM/PSM-Gateway/Main/Server/Guacamole/GuacBuild/protocols/rdp/RelWithDebInfo/guacsnd", client))
        //    guac_client_log(client, GUAC_LOG_WARNING,
    ...
    clrconv = static_cast<CLRCONV*>(calloc(1, sizeof(CLRCONV)));
    clrconv->alpha = 1;
    clrconv->invert = 0;
    clrconv->rgb555 = 0;
    clrconv->palette = static_cast<rdpPalette*>(calloc(1, sizeof(rdpPalette)));
    ...
    bitmap = static_cast<rdpBitmap*>(calloc(1, sizeof(rdpBitmap)));
    ...
    glyph = static_cast<rdpGlyph*>(calloc(1, sizeof(rdpGlyph)));
    ...
    pointer = static_cast<rdpPointer*>(calloc(1, sizeof(rdpPointer)));
...
static int rdp_guac_client_wait_for_messages(guac_client* client,
        int timeout_msecs) {
	static HANDLE handles[GUAC_RDP_MAX_FILE_DESCRIPTORS];
    guac_rdp_client* rdp_client = (guac_rdp_client*) client->data;
	...
	{
	   handles[i] = (HANDLE)read_fds[i];
	}
    DWORD dwResult = WaitForMultipleObjects(read_count, handles, false, timeout_msecs);
...
static int guac_rdp_handle_connection(guac_client* client) {
   ...
   guac_client_log(client, GUAC_LOG_DEBUG, "Starting RDP Handle Connection");
    guac_rdp_client* rdp_client = (guac_rdp_client*) client->data;
    guac_rdp_settings* settings = rdp_client->settings;
    ...
    guac_client_log(client, GUAC_LOG_DEBUG, "Trying to RDP Connect");
    ...
    guac_client_log(client, GUAC_LOG_DEBUG, "RDP Connection Successfull");
    /* Connection complete */
    ...
    guac_client_log(client, GUAC_LOG_DEBUG, "Starting Client Loop");
...
#ifdef HAVE_BOOST
		// Added for FPS Control
		// FPS is controlled by the guac config
		//if(client->client_fps > 0)
		//{
		//   boost::this_thread::sleep(boost::posix_time::milliseconds(1000 / client->client_fps));
		//}
#endif
    }
    guac_client_log(client, GUAC_LOG_DEBUG, "Client Loop Ended");
...
void* guac_rdp_client_thread(void* data) {
	...
	settings->drive_enabled = 0;
	//settings->drive_path = "C:/TEST";
