...
// TODO - Need Refactoring, no need to be swapped should be easy with boost
...
#ifdef HAVE_BOOST
	boost::mutex::scoped_lock lock(job->state_lock);
	job->state = state;
	job->state_modified.notify_all();
#elif defined HAVE_LIBPTHREAD
...
#ifdef HAVE_BOOST
			guac_rdp_print_blob blob = {job,buffer,length};
#elif defined HAVE_LIBPTHREAD
...
void* guac_rdp_print_job_alloc(guac_user* user, void* data) {
    ...
    guac_rdp_print_job* job = static_cast<guac_rdp_print_job*>(
		malloc(sizeof(guac_rdp_print_job)));
    if (job == NULL)
        return NULL;
    ...
    job->state = GUAC_RDP_PRINT_JOB_WAITING_FOR_ACK;
#ifdef HAVE_BOOST
	job->output_thread.reset(
		new boost::thread(boost::bind(guac_rdp_print_job_output_thread, job)));
#elif defined HAVE_LIBPTHREAD
